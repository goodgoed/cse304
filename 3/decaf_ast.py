...
class UnaryExpr(ASTNode):
    ...
    def __str__(self):
        return f"Unary({self.operator}, {self.operand})" if self.operator else f'{self.operand}'
...
class AssignExpr(ASTNode):
    ...
    def __str__(self):
        return f"Assign({self.left_operand}, {self.right_operand})"
...
class FieldAccessExpr(ASTNode):
    ...
    def __str__(self):
        return f"Field-access({self.base}, {self.field_name})"
...
class Method_Record():
    ...
    def __str__(self):
        res = f"METHOD: {self.id}, {self.name}, {self.containing_class}, {self.visibility}, {self.applicability}"
        res += f', {self.return_type}\n' if self.return_type.get_type() else '\n'
...
class Field_Record:
    ...
    def __str__(self):
        res = f"FIELD: {self.id}, {self.name}, {self.containing_class}, {self.visibility}, {self.applicability}, {self.type}"
        return res 
...
class Variable_Record:
    ...
    def __str__(self):
        res = f'VARIABLE: {self.id}, {self.name}, {self.kind}, {self.type}'
        return res
...
class Type_Record:
    def __init__(self, type):
        if type in ('int', 'float', 'string', 'boolean'):
            self.type = type
        elif type == 'void':
            self.type = ''
        else:
            self.type = f"user({type})"
...
class Symbol_Table():
    ...
    def lookup_field(self, field_name):
        ...
        while current_class and current_class['name']:
